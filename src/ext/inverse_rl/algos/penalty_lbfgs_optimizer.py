...
class PenaltyLbfgsOptimizer(Serializable):
    """
    Performs constrained optimization via penalized L-BFGS. The penalty term
    is adaptively adjusted to make sure that
    ...
    def update_opt(self, loss, target, leq_constraint, inputs,
        ...
        self._opt_fun = ext.lazydict(
            f_loss=lambda: tensor_utils.compile_function(inputs, loss,
                                                         log_name="f_loss"),
            f_constraint=lambda: tensor_utils.compile_function(inputs,
                                                               constraint_term,
    ...
    def optimize(self, inputs):
        ...
        cur_params = self._target.get_param_values(trainable=True).astype(
            'float64')
        ...
        for penalty_itr in range(self._max_penalty_itr):
            ...
            # already and no alternative parameter
