...
from ext.inverse_rl.algos.irl_batch_polopt import IRLBatchPolopt
from ext.inverse_rl.algos.penalty_lbfgs_optimizer import PenaltyLbfgsOptimizer
...
class IRLNPO(IRLBatchPolopt):
    ...
    def init_opt(self):
        ...
        old_dist_info_vars_list = [old_dist_info_vars[k] for k in
                                   dist.dist_info_keys]
        ...
        state_info_vars_list = [state_info_vars[k] for k in
                                self.policy.state_info_keys]

        if is_recurrent:
            valid_var = tf.placeholder(tf.float32, shape=[None, None],
                                       name="valid")
        else:
            valid_var = None
        ...
        if self.pol_ent_wt > 0:
            if 'log_std' in dist_info_vars:
                ...
                ent = tf.reduce_sum(log_std + tf.log(tf.sqrt(2 * np.pi * np.e)),
                                    reduction_indices=-1)
        ...
        if is_recurrent:
            ...
            surr_loss = - tf.reduce_sum(lr * adv * valid_var) / tf.reduce_sum(
                valid_var)
        else:
