...
class LatentSampler(object):
    '''
    Mixin class to be used when making a class intended to sample latent
    variables.
    ...
    def state_info_specs(self):
        '''
        All the inheriting classes can use this because we handle the setting
        up
    ...
    def merge_sym(self, obs_var, state_info_vars=None):
        '''
        Symbolically merges the input variable with the latent variable of
        the sampler
        ...
        with tf.variable_scope(self.name):
            if state_info_vars is not None and self.latent_name in \
                    state_info_vars.keys():
    ...
    def _build(self):
        with tf.variable_scope(self.name):
            self.latent = tf.placeholder(tf.float32, shape=(None, self.dim),
...
def _build_latent_variable_function(variable_type):
    ...
    if variable_type == 'categorical':
        ...
        raise ValueError(
            'variable_type not implemented: {}'.format(variable_type))
...
class UniformlyRandomLatentSampler(LatentSampler):
    ...
    def __init__(
        ...
        self.n_samples = None
        self._latent_variable_function = _build_latent_variable_function(
            variable_type)
    ...
    def encode(self, observations):
        '''
        For the case where the observations are available before hand,
        for example in
        the supervised case, this function allows for iterating the latent
        sampler to
        get the latent values at each timestep. This is essentially
        performing inference
        / recognition / encoding, so it's named encode to be symmetric with
        encoders.
    ...
    def get_action(self, observation):
        ...
        return copy.deepcopy(self.latent_values[0]), dict(
            latent=copy.deepcopy(self.latent_values[0]))
    ...
    def get_actions(self, observations):
        ...
        return copy.deepcopy(self.latent_values), dict(
            latent=copy.deepcopy(self.latent_values))
