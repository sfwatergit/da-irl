...
def plot_data(data, fig_name, value="AverageReturn"):
...
def get_datasets(fpath, condition=None):
    ...
    for root, dir, files in os.walk(fpath):
        if 'progress.csv' in files:
            ...
            exp_name = params['general_params']['run_id']
...
def main():
    ...
    parser.add_argument('--fig_dir', nargs='*')
    ...
    if isinstance(args.value, list):
        ...
        values = [args.value]
    for value in values:
        plot_data(data, fig_name='{}/{}.png'.format(args.fig_dir[0], args.exp_name[0]), value=value)
